...
struct myWalletView: View {
        ...
        NavigationView {
            VStack
            {
                    NavigationLink (destination: BalanceView()){
                        VStack{
                            Text("Balance:").font(.system(size: 44)).foregroundColor(.primary)
                            
                            Text(String(format: "R$ %.2f", balance)).font(.system(size: 32)).bold().foregroundColor(.primary)
                        }
                    }
    ...
    private func deleteItems(offsets: IndexSet) {
        withAnimation {
            ...
            balanceObj.value += items[0].value
            do {
